...
import { FORECAST_ACCOUNTS, type ForecastAccountRow } from "@/data/forecast-accounts";
import { FORECAST_MESSAGES, type ForecastMessageRow } from "@/data/forecast-messages";
...
export async function importAccountRows(
  ...
  for (const row of rows) {
    ...
    if (!memberIds.has(memberName)) {
      await ensureTeamMember(db, memberName, memberIds, result);
    }
...
export async function importMessageRows(
  db: SupabaseClient,
  rows: ForecastMessageRow[],
  result: ForecastMessageImportResult,
  memberIds: Map<string, string>,
    ...
    for (let i = 0; i < count; i++) {
      const notes = baseNotes
        ? count > 1
          ...
          : baseNotes
        : count > 1
          ? `OCR import (${i + 1}/${count})`
          : "Imported from sheet OCR";
...
export async function importForecastData(
  ...
  await importAccountRows(db, FORECAST_ACCOUNTS, result, memberIds, countryMap);
...
export async function importForecastMessages(
  ...
  await importMessageRows(db, FORECAST_MESSAGES, result, memberIds, {
